...
name: core-gate
description: >
  用 AI 量產遊戲/模擬/引擎/回測核心等中大型程式碼時的工程等級工作流:
  確定性核心契約 → 閘門先於功能 → 量產 → 機器驗收(靜態純度掃描+N-run 確定性+headless 情境閘門)→ 對抗審查。
  當使用者要 AI 幫忙寫遊戲引擎、模擬系統、數值核心、回測框架,或擔心「AI 生的程式碼沒人看得完怎麼辦」時使用——
  即使沒有明說「core-gate」也應自動觸發。
  觸發詞:工程等級、確定性核心、headless 驗收、merge gate、AI 生成大專案、模擬引擎、
  lockstep、replay、checksum、AI 幫我寫遊戲、遊戲引擎、回測核心、AI 量產程式碼、
  閘門、純度測試、deterministic、seed、遊戲驗收、AI 生 code 怎麼驗收。
  不適用:一次性腳本、純 UI、爬蟲膠水碼——這些直接寫就好,不需要工程等級工作流。
---
...
**適用**:遊戲、模擬、敘事引擎、ALife、確定性計算核心——任何「有一個該確定性的核心 + 一圈允許髒的外殼」的系統。
...
- **ML/表徵專案的確定性要驗到 embedding 層**:同輸入→逐位元同 embedding,先於任何下游指標。非確定性表徵餵給監督式下游會漏出假訊號(實測案例:eval 殘留未播種 randperm 製造假 Sharpe,修復後轉負)。
