---
name: validity-audit
description: 任何「有宣稱的產出」結案/發表/交付前的自我否證機制。兩段式:內部機械審計(按受審對象載領域包:軟體工程/內容教材/部署系統/文件)+ 強制獨立 reviewer(沒參與建造的 subagent 對抗式複查);可加選 T3 適用性審(為真但不夠好)。觸發詞:檢驗研究、發表前驗證、audit、coverage inflation、overfitting、驗收教材、審這個專案、結案體檢、宣稱核實。
---
...
| 受審對象 | 領域包 |
|---|---|
| 軟體工程/程式碼/系統/ML 宣稱(benchmark、測試覆蓋、效能、模型指標) | `references/mechanical-audit.md`(A–E:覆蓋灌水/benchmark/算術/統計/部署) |
...
3. **內部機械審計。** 按領域包逐項檢查+歷史挑戰,寫成 `audit/mechanical_audit.md`(或 `audit/content_audit.md`)。**每個 PASS 標記『建構必然(恆真)』還是『可能 FAIL』。**
...
- `references/mechanical-audit.md` — 軟體工程包 A–E(覆蓋灌水/benchmark/算術/統計/部署落差)。
